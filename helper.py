...
def pose2mat_inv(poses):
	mat = np.zeros([poses.shape[0],4,4])
	for idx, pose in enumerate(poses):
		mat[idx,0:3,0:3] = t3d.euler2mat(pose[5],pose[4],pose[3],'szyx')
		mat[idx,0:3,3] = pose[0:3]
		mat[idx,3,3] = 1
		mat[idx] = np.linalg.inv(mat[idx])
	return mat

###################### Voxel Operations #########################
# Create a voxel from point cloud (fitted in a unit cube with center at origin)
def voxelization(data_ip, size=32):
	# Arguments:
		# data:		Point Cloud (BxNx3)
		# size:		Size of the occupancy grid. (size of each voxel = unit_cube/size)
	# Output:
		# occupancy_grid:	Shape: [B x size x size x size]	(binary values for each voxel)
	
	data = np.copy(data_ip) + 0.5
	size_voxel = 0.03125

	occupancy_grid = np.zeros((data.shape[0], size, size, size))
	for idx, d in enumerate(data):
		
		for pt in d:
			x = int(pt[0]//size_voxel)
			y = int(pt[1]//size_voxel)
			z = int(pt[2]//size_voxel)
