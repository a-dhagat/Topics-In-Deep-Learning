...
class PoseEstimation(torch.nn.Module):
	...
	def voxelize(self, points, size=32, case=False):
		...
		#occupany_grid = torch.zeros((points_copy.shape[0], size, size, size), requires_grad=True)
		occupany_grid = torch.autograd.Variable(torch.empty(points_copy.shape[0], size, size, size), requires_grad=case)
		#print(occupany_grid)
		B, N, _ = points.shape
	...
	def forward(self, source, template, maxItr=8):
		...
		# Find Voxels of Template
		template_voxel = self.voxelize_numpy(template)
		template_voxel = template_voxel.unsqueeze(-1)
		template_voxel = template_voxel.permute(0,4,1,2,3).cuda()

		# Find features using voxels for template.
		template_feat = self.features(template_voxel)

		# Find voxels for source
		source_voxel = self.voxelize_numpy(source).cuda()
		source_voxel = source_voxel.unsqueeze(-1)
		source_voxel = source_voxel.permute(0,4,1,2,3)

		# Find features using voxels for source.
		...
		# Apply Matrix to source point cloud.
		...
		#template_voxel = self.voxelize(template)
		source_voxel = source_voxel.unsqueeze(-1)
		source_voxel = source_voxel.permute(0,4,1,2,3).cuda()
